...
export function ListingEditor({ initial, accessToken, onSave, onCancel }: ListingEditorProps) {
  ...
  return (
    <div className="rounded-xl border border-brand-500 bg-white p-5 dark:bg-slate-900">
      ...
      <div className="mt-5">
        <p className="text-sm text-slate-700 dark:text-slate-300">{tAdmin("images")}</p>
        {!initial ? (
          <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">{tAdmin("saveBeforePhotos")}</p>
        ) : (
