...
function OrdersPageContent() {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-12">
      ...
      {orders.length === 0 ? (
        <p className="mt-6 text-slate-600 dark:text-neutral-300">{t("empty")}</p>
      ) : (
        <div className="mt-6 space-y-4">
          {orders.map((order) => (
            <div
              key={order.id}
              className="rounded-xl border border-slate-200 bg-white p-5 dark:border-neutral-800 dark:bg-neutral-900"
            >
              ...
              <ul className="mt-3 space-y-1">
                {order.items.map((item) => (
                  <li key={item.id} className="text-sm text-slate-700 dark:text-neutral-300">
