  ...
  return (
    <AdminLayout>
      ...
      {isLoading ? (
        ...
        <p className="mt-6 text-sm text-slate-500 dark:text-neutral-400">{t("notFound")}</p>
      ) : (
        <div className="mt-4 max-w-2xl rounded-xl border border-slate-200 bg-white p-6 dark:border-neutral-800 dark:bg-neutral-900">
          <h1 className="text-2xl font-medium text-slate-900 dark:text-neutral-50">{t("title")}</h1>

          <dl className="mt-6 grid grid-cols-2 gap-4 text-sm">
            <div>
              <dt className="text-slate-500 dark:text-neutral-400">{t("dateLabel")}</dt>
              <dd className="text-slate-900 dark:text-neutral-50">{new Date(order.created_at).toLocaleString()}</dd>
            </div>
            <div>
              <dt className="text-slate-500 dark:text-neutral-400">{t("customerLabel")}</dt>
              <dd className="text-slate-900 dark:text-neutral-50">{order.email ?? "—"}</dd>
            </div>
          </dl>

          <div className="mt-6 border-t border-slate-200 pt-6 dark:border-neutral-800">
            <h2 className="text-sm font-semibold text-slate-900 dark:text-neutral-50">{t("itemsLabel")}</h2>
            <ul className="mt-3 space-y-1">
              {order.items.map((item) => (
                <li key={item.id} className="flex justify-between text-sm text-slate-700 dark:text-neutral-300">
                  <span>{item.service_name_snapshot}</span>
                  <PriceTag amountChf={Number(item.price_chf_snapshot)} />
                </li>
              ))}
            </ul>
            <div className="mt-3 flex justify-between border-t border-slate-100 pt-3 text-base font-medium text-slate-900 dark:border-neutral-800 dark:text-neutral-50">
          ...
          <div className="mt-6 border-t border-slate-200 pt-6 dark:border-neutral-800">
            <h2 className="text-sm font-semibold text-slate-900 dark:text-neutral-50">{t("paymentStatusLabel")}</h2>
          ...
          <div className="mt-6 border-t border-slate-200 pt-6 dark:border-neutral-800">
            <h2 className="text-sm font-semibold text-slate-900 dark:text-neutral-50">{t("fulfillmentStatusLabel")}</h2>
            <p className="mt-1 text-slate-700 dark:text-neutral-300">
